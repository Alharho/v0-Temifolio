...
function Sidebar({ active, setActive }: { active: View; setActive: (view: View) => void }) {
  return <aside className="sidebar"><div className="brand"><div className="brand-mark">P</div><div><strong>PUNTA</strong><span>INTELLIGENCE</span></div></div><div className="workspace-label">WORKSPACE <span>PERSONAL</span></div><nav>{navItems.map(({ id, label, icon: Icon }) => <button key={id} className={active === id ? 'nav-active' : ''} onClick={() => setActive(id)}><Icon />{label}{id === 'live' && <span className="nav-live">1</span>}</button>)}</nav><div className="sidebar-bottom"><div className="data-health"><div><span className="health-icon"><Activity /></span><span><strong>Data health</strong><small>All systems nominal</small></span></div><Signal>98.4% fresh</Signal></div><button className="sidebar-link"><CircleHelp /> Documentation</button><div className="profile"><div className="avatar">TO</div><span><strong>Temitope O.</strong><small>Analyst workspace</small></span><ChevronRight /></div></div></aside>
}

function Topbar({ view, setView }: { view: View; setView: (view: View) => void }) {
  return <header className="topbar"><div className="mobile-brand"><div className="brand-mark">P</div><strong>PUNTA</strong></div><div className="crumb"><span>Workspace</span><ChevronRight /><strong>{navItems.find((item) => item.id === view)?.label}</strong></div><div className="top-actions"><button className="date-button"><CalendarDays />{PRODUCT.dateLabel}<span className="timezone">{PRODUCT.timezone}</span></button><button className="icon-button"><Search /></button><button className="icon-button notification"><Bell /><i /></button><div className="top-avatar">TO</div></div></header>
}
