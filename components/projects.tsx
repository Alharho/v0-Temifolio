...
const projects = [
  ...
  },
  {
    ...
    description: 'UI/UX and visual identity design for a real-time photo sharing application.',
  ...
  },
  {
    ...
    description: 'Complete brand identity and comprehensive logo style guide for a technology company.',
  ...
  },
  {
    ...
    description: 'Comprehensive brand guide and social-media design strategy to strengthen digital presence.',
    tags: ['Brand Identity', 'Social Media', 'Strategy'],
  ...
  },
  {
    ...
    description: 'Social media campaign graphics driving 50% increase in engagement and 30% boost in sales.',
  ...
  },
  {
    ...
    company: 'Kortis Legends',
    description: 'High-impact event branding and tournament campaign visuals for the gaming community.',
  ...
  return (
    <section id="projects" className="relative py-28 sm:py-36">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        >
          <div>
            <span className="text-xs text-primary tracking-[0.2em] uppercase font-medium">Selected Work</span>
            <h2 className="font-serif text-4xl sm:text-5xl lg:text-6xl font-semibold mt-4 text-white tracking-tight">
          ...
          <a
            ...
            className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/[0.03] border border-white/[0.08] text-sm text-gray-400 hover:text-white hover:border-white/20 transition-all duration-300"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {projects.map((project, i) => (
            <motion.a
              ...
              className="group bg-white/[0.02] rounded-2xl border border-white/[0.06] hover:border-white/[0.12] transition-all duration-500 overflow-hidden block"
            >
              {/* Thumbnail */}
              <div className="relative overflow-hidden aspect-[4/3]">
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-gray-800 to-gray-900 items-center justify-center hidden">
                  <span className="font-serif text-5xl font-bold text-primary/20">{project.company[0]}</span>
                </div>
                {/* Overlay on hover */}
                <div className={`absolute inset-0 bg-[#0d0d0d]/70 flex items-center justify-center transition-opacity duration-500 ${hoveredIdx === i ? 'opacity-100' : 'opacity-0'}`}>
                  <div className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-primary text-black text-sm font-medium">
              ...
              {/* Content */}
              <div className="p-6">
                <p className="text-xs text-primary mb-2 tracking-wide font-medium">{project.company}</p>
                <h3 className="font-semibold text-white text-lg leading-snug mb-3 group-hover:text-primary/90 transition-colors duration-300">
                  {project.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tags.map(tag => (
                    <span key={tag} className="px-2.5 py-1 rounded-full bg-white/[0.04] text-xs text-gray-400 border border-white/[0.06]">
                      {tag}
                    </span>
                  ))}
                </div>
